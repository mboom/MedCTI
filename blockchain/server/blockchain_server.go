...
func (bs *blockchainServer) PublishLogData(_ context.Context, flow *pb.Flow) (*empty.Empty, error) {
	// open the file that contains the ledger
	...
	// convert a flow message to a byte stream
	data, err := proto.Marshal(flow)
	if err != nil {
		panic(err)
	}

	// calculate the size of the byte stream
	size := make([]byte, 4)
	...
	// insert the 4-byte size before the byte stream
	data = size + data
...
func (bs *blockchainServer) FetchLogData(_ *empty.Empty, stream pb.Blockchain_FetchLogDataClient) error {
	// open the file that contains the ledger
	...
	for err == nil {
		...
		// convert the byte stream to a flow message
		flow := &pb.Flow{}
